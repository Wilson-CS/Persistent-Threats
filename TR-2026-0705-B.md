...
The configuration transform file (`UEMSAgent.mst`) and embedded JSON metadata linked the agent to the attacker's management console with the following highly specific cryptographic, connectivity, and hosting parameters:
* **Primary Command & Control (C2) Server:** `202.61.160.208:8383`
  * **Hosting Provider:** CTG Server Ltd.
  * **Routing / BGP Route:** Rackip Consultancy Pte. Ltd. / BGP Consultancy Pte Ltd
  * **Autonomous System Number (ASN):** `AS152194` (and/or `AS64050`)
  * **Geographic Registry:** Singapore (`SG`) with abuse and administrative nodes pointing to Hong Kong (`HK`)
* **Backup Command & Control (C2) Server:** `202.61.160.202:8383`
  * **Hosting Provider:** CTG Server Ltd.
  * **Routing / BGP Route:** Rackip Consultancy Pte. Ltd. / BGP Consultancy Pte Ltd
  * **Autonomous System Number (ASN):** `AS152194` (and/or `AS64050`)
  * **Geographic Registry:** Singapore (`SG`) with abuse and administrative nodes pointing to Hong Kong (`HK`)
* **Adversary Infrastructure Domain:** `haomls.pro`
  * **Staging CDN/DNS Proxy:** Cloudflare, Inc. (Proxied IPs: `172.67.154.246`, `104.21.6.56`)
...
| Indicator / Full URL | Type | Purpose / Hosting Details |
| :--- | :--- | :--- |
| `202.61.160.208:8383` | IPv4 Socket | Primary C2 Server (Host: CTG Server Ltd., Route: Rackip Consultancy, ASN: AS152194, Loc: SG/HK) |
| `202.61.160.202:8383` | IPv4 Socket | Backup C2 Server (Host: CTG Server Ltd., Route: Rackip Consultancy, ASN: AS152194, Loc: SG/HK) |
| `https://haomls.pro/wdwd/lio.png` | HTTPS URL | Obfuscated Stage 2 Loader (Masked as PNG; Domain behind Cloudflare) |
| `https://haomls.pro/caiwuaaa/ascaaae.zip` | HTTPS URL | Stage 3 Payload Package (UEMSAgent MSI Bundle; Domain behind Cloudflare) |
| `haomls.pro` | DNS Domain | Adversary Infrastructure Domain (Proxied via Cloudflare: 172.67.154.246 / 104.21.6.56) |
